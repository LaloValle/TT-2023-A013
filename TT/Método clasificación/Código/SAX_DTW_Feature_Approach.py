"""
    ...
    Using a SAX symbolic representation for every time series, a SAX-DTW-Feature vector is arranged by computing the DTW distance of
    the SAX representations of the input time series and each class representative already in it's SAX representation for the 
    classification using either SVC(Support Vector Classifier) or Softmax(Multiclass Logistic Regression Classifier)

    Each class representative is calculated using the DTW Barycenter Averaging method after the training instances of each class has
    already been transformed into a SAX time series.
