...
from LLibraries.Tools import euclidean_distance
...
def DTW(series_1, series_2, window_size:int=0, distance=euclidean_distance):
    ...
    return sqrt(DTW[len(series_1)][len(series_2)])

def optimal_warping_path(series_1, series_2, window_size:int=0, distance=euclidean_distance):
    """Computes the optimal warping path resulted of the algorithm DTW 
...
def DTW_complete(series_1, series_2, window_size:int=0, distance=euclidean_distance):
    """Computes the algorithm DTW and returns the DTW distance measure and the optimal warping path
...
def DTW_1NN(classification_set:list,training_set:list,window_size:int=0,lb_keogh_r:int=5,**kargs):
    ...
    """
    predictions=np.array([]); dtw_calculated = 0
    ...
    for i in range(len(classification_set)):
        ...
        for training_series in training_set:
            ...
            if LB_Keogh(classification_series[1:],training_series[1:],lb_keogh_r) < minimum_distance:
                ...
                distance = DTW(classification_series[1:],training_series[1:],window_size)
                dtw_calculated += 1
        ...
        predictions = np.append(predictions,int(closest_sequence_label))
