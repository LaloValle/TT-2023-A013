...
def minimum_warping_window(time_series_set,prediction_function,alphabet_size:int=0,number_iterations:int=10,upper_bound_window:int=0,lower_bound_window:int=2,representatives:dict=dict(),convert_int:bool=False,verbose:bool=False,k=10):
    ...
    table_WindowSize_vs_Accuracy = pd.DataFrame(WindowSize_vs_Accuracy, index=[f'{i+1}°' for i in range(number_iterations)]+['Results'], columns=list(range(lower_bound_window,upper_bound_window+1)))
    # Saves the array
    date = datetime.now()
    table_WindowSize_vs_Accuracy.to_csv(f'Results/WindowSize_vs_Accuracy-{date.day}-{date.month}-{date.year}_{date.hour}-{date.minute}.csv')
